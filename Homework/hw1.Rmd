...
```
Our mean squared error on the training data is 1934162; not great. Of course, we could have included all the other variables and reduced our MSE but we would have wantonly included covariates without minimally examining that the assumptions of the model are checked.
...
design_matrix <- model.matrix(apps ~ .-1, data = train)
...
Our lambda that minimizes our MSE is ~11. We will use this in our ridge model.

```{r}
lambda_cv$lambda.min
```

Below we see that our ridge model has a worse MSE than our simple linear model. In fact, the MSE has increased by 139564.

```{r}
ridge <- glmnet(design_matrix, 
                train$apps, alpha = 0, lambda = lambda_cv$lambda.min)

test_design_matrix <- model.matrix(apps ~ . -1, data = test)
...
Our lambda that minimizes our MSE is ~19 in the LASSO setting. We will use this in our LASSO model.

```{r}
lambda_cv$lambda.min
```

Below we see that our lasso model has a worse MSE than our simple linear model. In fact, the MSE has increased by 221342.

```{r}
lasso <- glmnet(design_matrix, 
                train$apps, alpha = 1, lambda = lambda_cv$lambda.min)
...
# Check that we have 1089 rows.
nrow(df)

# Check that we have 21 years worth.
df %>% select(year) %>% table # yup!
```

### (a) Produce some numerical and graphical summaries of the Weekly data. Do there appear to be any patterns?

```{r, echo = FALSE, include = FALSE, eval = FALSE}
ggplot(data = df, mapping = aes(x = year, y = n, color = species_id)) +
    geom_line()
